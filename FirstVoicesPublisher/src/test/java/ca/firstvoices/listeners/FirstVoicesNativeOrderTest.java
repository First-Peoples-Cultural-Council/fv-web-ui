...
@Deploy({"FirstVoicesData", "org.nuxeo.ecm.platform", "org.nuxeo.ecm.platform.commandline.executor",
        ...
        "org.nuxeo.ecm.platform" + ".video.core", "org.nuxeo.ecm.platform.audio.core", "org.nuxeo.ecm.automation" +
        ".scripting", "FirstVoicesNuxeoPublisher" + ".tests:OSGI-INF/extensions/ca.firstvoices.fakestudio.xml",
        "FirstVoicesNuxeoPublisher:OSGI-INF/extensions/ca" + ".firstvoices.templates.factories.xml",
        "FirstVoicesSecurity:OSGI-INF/extensions/ca.firstvoices.operations" + ".xml", "FirstVoicesNuxeoPublisher:OSGI"
        + "-INF/extensions/ca.firstvoices.nativeorder.services.xml"})
public class FirstVoicesNativeOrderTest {
    ...
    public void testDialectOrderingNisgaa() throws Exception {
        ...
        DocumentModelList docs =
                session.query("SELECT * FROM FVWord WHERE ecm:ancestorId='" + dialect.getId() + "' " + "ORDER BY " +
    ...
    public void testDialectOrderingPhrases() throws Exception {
        String[] orderedPhrases = {"À", "Á", "Â", "Ã", "Ä", "Å", "Æ", "Ç", "È", "É", "Ê", "Ë", "Ì", "Í", "Î", "Ï",
                ...
                + " you play with fire, you will get burned", "Judge not, that " + "ye be not judged", "Kill two " +
                "birds " + "with one stone.", "Learn a language, and you will avoid a " + "war", "Memory is the " +
                "treasure of the " + "mind", "No man is an island", "Oil and water do not mix", "Penny, Penny. Makes " +
                "many.", "Respect is " + "not given, it is earned.", "Sticks and stones may break" + " my bones, but " +
                "words will never hurt me" + ".", "There is no smoke without fire.", "Use it or lose it", "Virtue is " +
                "its own reward", "When it " + "rains it pours.", "You cannot teach an old dog new tricks", "Zeal " +
