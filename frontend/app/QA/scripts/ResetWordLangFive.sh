...
java -jar fv-batch-import-*.jar -url "$TARGET/nuxeo" -username $CYPRESS_FV_USERNAME -password $CYPRESS_FV_PASSWORD -domain FV -csv-file $DIRECTORY/app/QA/scripts/files/testLangFiveWord.csv -data-path $DIRECTORY/app/QA/scripts/files/testLangTwoMedia/ -dialect-id fillerID -language-path Test/Test/TestLanguageFive 1>/dev/null
if [[ "$?" -ne 0 ]]; then
...
java -jar fv-batch-import-phrases.jar -url "$TARGET/nuxeo" -username $CYPRESS_FV_USERNAME -password $CYPRESS_FV_PASSWORD -domain FV -csv-file $DIRECTORY/app/QA/scripts/files/testLangFivePhrase.csv -data-path $DIRECTORY/app/QA/scripts/files/testLangTwoMedia/ -dialect-id fillerID -language-path Test/Test/TestLanguageFive 1>/dev/null
if [[ "$?" -ne 0 ]]; then
