...
if [ "$1" != "-skip-clone" ]; then
    ...
    FVUTILS=$DIRECTORY/app/QA/temp/fv-utils-temp
    FVBATCH=$DIRECTORY/app/QA/temp/fv-batch-import-temp
...
java -jar fv-batch-import-*.jar -url "$TARGET/nuxeo" -username $CYPRESS_FV_USERNAME -password $CYPRESS_FV_PASSWORD -domain FV -csv-file $DIRECTORY/app/QA/scripts/files/DevLangOneWords.csv -data-path $DIRECTORY/app/QA/scripts/files/testLangTwoMedia/ -dialect-id fillerID -language-path Test/Test/DevLangOne
if [[ "$?" -ne 0 ]]; then
...
java -jar fv-batch-import-phrases.jar -url "$TARGET/nuxeo" -username $CYPRESS_FV_USERNAME -password $CYPRESS_FV_PASSWORD -domain FV -csv-file $DIRECTORY/app/QA/scripts/files/DevLangOnePhrases.csv -data-path $DIRECTORY/app/QA/scripts/files/testLangTwoMedia/ -dialect-id fillerID -language-path Test/Test/DevLangOne
if [[ "$?" -ne 0 ]]; then
...
cd $DIRECTORY/app/QA/scripts/batch_jarfiles/
java -jar fv-batch-import-alphabet.jar -url "$TARGET/nuxeo" -username $CYPRESS_FV_USERNAME -password $CYPRESS_FV_PASSWORD -domain FV -csv-file $DIRECTORY/app/QA/scripts/files/alphabet.csv -data-path $DIRECTORY/app/QA/scripts/files/testLangTwoMedia/ -dialect-id fillerID -language-path Test/Test/DevLangOne
if [[ "$?" -ne 0 ]]; then
