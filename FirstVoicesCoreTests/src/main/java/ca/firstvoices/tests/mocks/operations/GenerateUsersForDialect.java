...
@Operation(id = GenerateUsersForDialect.ID, category = Constants.GROUP_NAME, label =
    "Generate Users for Dialect", description = "Operation to generate users given 1 mock data test dialect ")
public class GenerateUsersForDialect {
  ...
  public void run() {
    ...
    if (session.exists(path)) {
      generateDialectUsersService.generateUsersForDialect(session, path);
    } else {
